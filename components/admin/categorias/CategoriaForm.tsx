...
import { criarCategoria, atualizarCategoria } from "@/lib/actions/admin-categorias";
...
export function CategoriaForm({ categoria }: CategoriaFormProps) {
  ...
  const [exibirHome, setExibirHome] = useState(categoria?.exibir_home ?? true);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        ...
        if (categoria) await atualizarCategoria(categoria.id, input);
        else await criarCategoria(input);
      } catch (err) {
